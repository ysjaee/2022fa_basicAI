...
eps = 1e-7
cost = encode_y * np.log(hypothesis + eps)
cost = -cost.mean()
print('cost: ', cost)
...
while True:
    ...
    if cost < 0.0001:
        break
    w_grad = (1/m)*np.dot(x_normal.T, (hypothesis - encode_y))
    b_grad = (1/m)*np.sum(hypothesis - encode_y)
...
for i in range(10000):
    ...
    print('label: ', encode_y[random_num])
    print('----' * 20)
    if prediction == np.argmax(encode_y[random_num]):
        success_cnt += 1
        print('success')
    else:
        print('fail')
